...
func newReplicationBatchWorker(nodeID string, client pb.KVClient) *replicationBatchWorker {
	w := &replicationBatchWorker{
		nodeID:  nodeID,
		client:  client,
		enqueue: make(chan replicationTask, 1024), // tighter bound
		stopCh:  make(chan struct{}),
	}
	go w.run()
	return w
...
func (w *replicationBatchWorker) run() {
	...
	// dedicated recv loop (true pipeline)
	ackCh := make(chan error, maxInFlightBatches)
